...
const GamesPage: React.FC = () => {
  ...
  const priceRange = prices.length ? `₵${Math.min(...prices)}-₵${Math.max(...prices)}` : 'N/A';
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 md:gap-8 mb-10 md:mb-16">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <span className="text-primary font-pixel text-[8px] md:text-[10px] tracking-widest uppercase">Simulation_Library_V4</span>
            </div>
            <h1 className="text-3xl sm:text-5xl md:text-7xl font-pixel text-white leading-none">
              <GlitchText text="GAME" color="white" persistent /><br/>
              <span className="text-secondary neon-glow-magenta"><ShuffleText text="DATABASE" delay={400} /></span>
            </h1>
          </div>
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="p-4 md:p-6 space-y-3 md:space-y-4 flex-grow flex flex-col">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="text-[8px] font-pixel text-primary mb-1 tracking-widest uppercase">{game.category}</div>
                        <h3 className="text-lg font-display font-black text-white group-hover:text-primary transition-colors">
                          {game.title}
                        </h3>
                      </div>
                      <div className="text-right">
                        <div className="text-[8px] font-pixel text-slate-600 mb-1">PRICE</div>
                        <div className="text-sm font-display font-black text-primary">₵{game.price}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                      <div>
                        <div className="text-[8px] font-pixel text-slate-600 mb-1">INTENSITY</div>
                        <div className={`text-[10px] font-pixel ${
                          game.intensity === 'MAX' ? 'text-secondary' : 
                          game.intensity === 'HIGH' ? 'text-orange-500' : 
                          game.intensity === 'MED' ? 'text-primary' : 'text-green-500'
                        }`}>
                          {game.intensity}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-[8px] font-pixel text-slate-600 mb-1">RUNNERS</div>
                        <div className="text-[10px] font-pixel text-white">{game.players}</div>
                      </div>
                    </div>
        ...
        <div className="mt-12 md:mt-20 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {[
            ...
            <div key={stat.label} className="p-4 border border-dashed border-white/10 flex flex-col items-center justify-center bg-white/5">
              <div className="text-[8px] font-pixel text-slate-600 mb-2 tracking-widest">{stat.label}</div>
              <div className="text-sm font-display font-bold text-primary tracking-tighter">{stat.value}</div>
            </div>
          ))}
        </div>
      </div>
